...
public partial class Player : CharacterBody3D
{
    ...
    [ExportCategory("PlayerMovementSettings")] [Export]
    float GRAVITY = 9.8f * GRAVITY_MULTIPLIER;
    ...
    [ExportCategory("CameraSettings")] [Export]
    private float cameraLookSensitivity = 0.006f;
    ...
    [Export] private bool firstPerson = false;
    ...
    public override void _Ready()
    {
        ...
        camera1P = GetNode<Camera3D>("%Camera1P");
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        if (@event is InputEventMouseMotion motion && MouseIsCaptured)
        {
            ...
            if (firstPerson)
            {
                ...
                camera1P.RotateX(-motion.Relative.Y * cameraLookSensitivity);

                Vector3 cameraRot = camera1P.Rotation;
                cameraRot.X = Mathf.Clamp(cameraRot.X, Mathf.DegToRad(-lookUpDegrees), Mathf.DegToRad(lookUpDegrees));
                camera1P.Rotation = cameraRot;
            }
            else
            {
                // this.RotateY(-motion.Relative.X * cameraLookSensitivity);
    ...
    private void FOVJuice(double delta)
    {
        ...
        camera1P.Fov = camera1P.Fov.Lerp(targetFOV, delta * fovJuiceWeight);
    }
    ...
    private void HeadBob(double delta)
    {
        ...
        var camTran = camera1P.Transform;

        var pos = Vector3.Zero;
        pos.Y = Mathf.Sin(t_bob * bob_Speed) * bob_Height;
        camTran.Origin = pos;
        camera1P.Transform = camTran;
    }
    ...
    private void ChangePerspectives()
    {
        ...
        if (was)
        {
            arm.Rotation = arm.Rotation with { X = camera1P.Rotation.X };
        }
        else
        {
            camera1P.Rotation = arm.Rotation with { X = arm.Rotation.X };
        }

        // AnchorGunToCamera();

        // camera3P.Current = !firstPerson;
        camera1P.Current = firstPerson;
    }
