class_name ControllerInputViewer
extends Node2D
...
@export var Console : ConsoleType = ConsoleType.PlayStation;


func _ready():
	#Inicial position
	self.position = Vector2(ScreenWidth/2, ScreenHeight/2)
	var parent = get_parent() as ControllerViewerCaller;
	parent.ControllerHud = self;
...
#Set Triggers Value
	...
	if Input.is_action_just_pressed("Debug_ControllerInputViewer_Switch"):
		_on_console_button_pressed();
	...
	# Set console label
	$Buttons/ConsoleButton.text = str(ConsoleType.find_key(Console))
