...
func reload_scene() -> void:
	print("[ReloadScene] Reload initialized", Time.get_time_string_from_system())
	var edited_scene: Node = EditorInterface.get_edited_scene_root()
	print("[ReloadScene] Edited scene", "%s.scene_file_path" % edited_scene, edited_scene.scene_file_path)
	EditorInterface.reload_scene_from_path(edited_scene.scene_file_path)
	print("[ReloadScene] Scene reloaded", Time.get_time_string_from_system())
